...
Mermaid コードを SVG / PNG 画像に変換する HTTP API です。TypeScript + Express で `/render`、`/healthz`、`/livez`、`/readyz`、`/metrics` を提供し、既定では `@mermaid-js/mermaid-cli` の Programmatic API と常駐 BrowserPool でレンダリングします。

## 特徴

### Mermaid 本家の foreignObject テキスト中央寄せバグを修正

本家 Mermaid 11.15.0 では、ノード内のテキストは `<foreignObject>` 内の `<div>` を `display: table-cell` でレンダリングしますが、`i` のような幅の狭いグリフ、`W` のような幅の広いグリフ、絵文字、CJK 句読点を含むと、矩形の幾何学的中央から右方向へずれて描画されます。本 API は SVG を後処理で flex コンテナ (`display: flex; justify-content: center; align-items: center`) でラップし、SVG・PNG 両方の出力で真の中央寄せを復元します。

| 未修正 (生 Mermaid 11.15.0) | 本 API 経由 |
|---|---|
...
赤の破線はノードの `rect` の実測中心線です。「未修正」側ではテキストが線より右へ顕著にずれ、「本 API 経由」側では線上に整列します。14 種類のダイアグラム回帰内訳は `docs/foreignobject-inner-centering-verification-2026-05-17.md` を参照してください。

### その他の特徴

- **リクエスト単位の PNG 解像度指定**: `scale: 1..4` (既定 `3`) をリクエスト時に指定 — 環境変数不要。
- **Mermaid 本家準拠の既定値**: `flowchart.useMaxWidth=true`、`diagramPadding=8`、`nodeSpacing=rankSpacing=50`。HTML 埋め込み時に SVG が親要素に自然にフィットします。
- **Chromium sandbox 有効**: `--no-sandbox` 不使用。コンテナは非 root 実行、read-only filesystem、tmpfs、capability drop、PID / メモリ制限でハードニング済み。
- **常駐 BrowserPool**: 永続化された Chromium コンテキストによる低レイテンシ描画。
- **防御的バリデーション**: リクエスト単位の `mermaid_config` override をサーバ側スキーマで検証し、prototype-pollution 対策を実施。

## 前提

- Node.js 20
- Docker Desktop(WSL 利用可)
...
Windows で Docker Desktop を有効化し、対象の WSL ディストリで以下を実行します。WSL から Docker Desktop を呼び出している場合も実体は Windows Docker Desktop (LinuxKit VM) なので、本番と同じ overlay 併用が必須です。
...
`POST /render` は JSON ボディを受け取り、バイナリ画像 (`image/svg+xml` または `image/png`) を返します。

### リクエスト本文

| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `code` | string | ✓ | Mermaid ソースコード。最大 `MAX_CODE_SIZE` バイト(既定 51200)。 |
| `format` | `"svg"` \| `"png"` | 任意 | 出力形式。既定 `svg`。 |
| `timeout_ms` | integer | 任意 | レンダリングタイムアウト(ミリ秒)。未指定時はサーバ既定。 |
| `scale` | integer | 任意 | PNG の解像度倍率 (`deviceScaleFactor`)。**1〜4** の整数、サーバ既定値 **3**。`format=svg` と同時送信した場合は無視され、200 OK で `scale` 未指定時と同一の SVG が返る(構造化ログに `scale_ignored_for_svg` 警告を記録)。範囲外 / 非整数 → `400 invalid_request` (`error_field: "scale"`)。 |
| `mermaid_config` | object | 任意 | リクエスト単位の Mermaid 設定 override(サーバ側スキーマで検証)。 |
| `post_process` | object | 任意 | SVG 後処理オプション(`strip_max_width`、`rewrite_ids` など)。 |
...
`code` / `format` / `timeout_ms` / `scale` / `mermaid_config` などが不正な場合は `400 invalid_request` を返します。JSON 本文には `error_type` / `error_field` / `error_constraint` が含まれます。
...
`.env` で調整可能です(Docker 起動時に読み込まれます)。

- `DEFAULT_TIMEOUT_MS`: Mermaid CLI のタイムアウト(ミリ秒、デフォルト 8000)
- `RATE_LIMIT_MAX_INFLIGHT`: HTTP 層の同時受付上限(デフォルト 15)
- `BROWSER_POOL_SIZE`: BrowserContext プールサイズ(デフォルト 4)
- `POOL_QUEUE_MAX`: BrowserPool の待ちキュー上限(デフォルト 20)
- `MAX_CODE_SIZE`: `code` の最大バイト数(デフォルト 51200)
- `RENDERER_MODE`: `programmatic` または `cli`(デフォルト `programmatic`)

> PNG の解像度倍率と SVG の padding は環境変数ではなくなりました。PNG `scale` はリクエスト単位の API パラメータ(`1..4`、既定 `3`)で指定します。旧 `PNG_RENDER_SCALE` / `MERMAID_PADDING` 環境変数は廃止されました。

## 依存更新ポリシー

`@mermaid-js/mermaid-cli` は Programmatic API が semver 対象外のため exact pin で管理します。依存更新時は `npm ci` で lockfile を同期し、property test、統合テスト、画像差分確認、性能確認を通してから反映してください。
...
本番運用 (Windows Docker Desktop) では `--no-sandbox` を使用せず、`docker-compose.dev-sysadmin.yml` overlay で `SYS_ADMIN` / `SYS_CHROOT` capability を追加して Chromium 自身の Linux namespace sandbox を有効化します。非 root 実行、`chromium-sandbox`、`tini`、read-only filesystem、tmpfs、PID / メモリ制限はベース構成 (`docker-compose.yml`) で固定されており、overlay はそれらを維持したまま sandbox 起動に必要な最小権限のみを追加します(`requirements.md` C-P-09、`design.md` §8.1 参照)。
...
日本語表示の美観を優先するため、コンテナには `fonts-noto-cjk` を導入し、Mermaid 設定で `Noto Sans CJK JP` を優先します。
...
REQ-U-11 (2026-05-18) 時点:

- `npm run build`: pass
- `npm test`: 58 files / 245 tests pass
- `npm audit --omit=dev --audit-level=high`: pass
- `docker compose build`: pass
- Windows Docker Desktop で `docker-compose.dev-sysadmin.yml` 併用起動下、`/healthz` / `/readyz` / `/livez` / `/render` SVG・PNG smoke が 200
- 性能 NFR-01 (単純 flowchart p50 ≤ 500ms): PASS (`docs/perf/2026-05-16_compare.md` 参照)
- Phase 6 切替後 5 分監視: PASS (`docs/phase6-deployment-verification/5min-monitoring-2026-05-16.md` 参照)
